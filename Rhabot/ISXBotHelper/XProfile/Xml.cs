...
namespace ISXBotHelper.XProfile
{
	/// <summary>
	...
	internal class Xml : XmlBased
	{
        public class XAttribute
        {
            ...
            public XAttribute(string attributeName, string attributeValue)
            {
		...
		{
            SetValue(section, entry, value, null);
		}
        ...
        /// </summary>
		/// <param name="section">
		///   The name of the section that holds the entry. 
        /// </param>
		/// <param name="entry">
		///   The name of the entry where the value will be set. 
        /// </param>
		/// <param name="value">
		///   The value to set. If it's null, the entry is removed. 
        /// </param>
        /// <param name="EntryAttributes">Optional list of additional attributes. Send null for no attributes</param>
		/// <exception cref="InvalidOperationException">
		///   <see cref="Profile.Name" /> is null or empty, 
		///   <see cref="Profile.ReadOnly" /> is true, or
		///   the resulting XML document is invalid. </exception>
		...
		///   XML file until the buffer is flushed (or closed). </para></remarks>
		/// <seealso cref="GetValue" />
        public void SetValue(string section, string entry, object value, List<XAttribute> EntryAttributes)
        {
                ...
                writer.WriteStartElement(m_rootName);
                    ...
                    writer.WriteAttributeString("name", null, section);
                        writer.WriteStartElement("entry");
                            ...
                            if ((EntryAttributes != null) && (EntryAttributes.Count > 0))
                            {
                                // loop through and add each attribute
                	            foreach (XAttribute xat in EntryAttributes)
                                    writer.WriteAttributeString(xat.AttributeName, null, xat.AttributeValue);
                            }
            ...
            if (entryNode == null)
            {
                ...
                if ((EntryAttributes != null) && (EntryAttributes.Count > 0))
                {
                    ...
                    foreach (XAttribute xat in EntryAttributes)
                    {
                        attribute = doc.CreateAttribute(xat.AttributeName);
                        attribute.Value = xat.AttributeValue;
                        element.Attributes.Append(attribute);
                    }                        
                }
        ...
        /// <summary>
        /// Gets the attribute value of an attribute for this section/entry
        /// </summary>
        /// <param name="section">
        ///   The name of the section that holds the entry with the value. 
        /// </param>
        /// <param name="entry">
        ///   The name of the entry where the value is stored. 
        /// </param>
        /// <param name="attributeName">the name of the attribute to retrieve the value from</param>
        public string GetValue_Attribute(string section, string entry, string attributeName)
        {
            VerifyAndAdjustSection(ref section);
            VerifyAndAdjustEntry(ref entry);
            ...
            try
            {
                ...
                XmlNode entryNode = root.SelectSingleNode(GetSectionsPath(section) + "/" + GetEntryPath(entry));
                return entryNode.Attributes[attributeName].Value;
            }
            catch
            {
                return null;
