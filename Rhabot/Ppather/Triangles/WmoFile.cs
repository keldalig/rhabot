...
namespace Wmo
{
    unsafe static class ChunkReader
    {
    ...
    public class WMOInstance
    {
        ...
        public WMOInstance(WMO wmo, System.IO.BinaryReader file)
        {
            ...
            file.ReadInt16(); // short crap = file.ReadInt16();
        }
    }
    ...
    public abstract class Manager<T>
    {
        ...
        Dictionary<string, int> items_LRU = new Dictionary<string, int>();

        int NOW = 0;
        int maxItems;

        protected Manager(int maxItems)
        {
        ...
        public void EvictIfNeeded()
        {
            ...
            if (toEvict != null)
            {
//                //Console.WriteLine("Drop item : " + toEvict);
        ...
        {
            path = path.ToLower(System.Globalization.CultureInfo.CurrentCulture);
            T w = Get(path);
            if (w == null)
            {
                EvictIfNeeded();
    ...
    {      
        ...
        public override Model Load(String path)
        {
            ...
            string localPath = "model.tmp";
            if (set.ExtractFile(file, localPath))
            {
                Model w = new Model();
                w.fileName = file;
                new ModelFile(localPath, w);
                return w;
            }
            return null; 
        }
    ...
    public class ModelInstance
    {
        ...
        public ModelInstance(Model m, System.IO.BinaryReader file)
        {
            ...
            model = m;
            file.ReadUInt32(); //uint d1 = file.ReadUInt32();
            {
                float f0 = file.ReadSingle();
                float f1 = file.ReadSingle();
                float f2 = file.ReadSingle();
    ...
    public class ModelFile : IDisposable
    {
        ...
        public ModelFile(string path, Model m)
        {
            ...
            {
                using (file = new System.IO.BinaryReader(stream))
                {
                    try
                    {
                        ReadHeader();
        ...
        private void ReadHeader()
        {
            ...
            {
                //Console.WriteLine("Bad header in m2 file: " + header); 
                return; 
            }
            ...
            model.view = ReadViews(nViews, ofsViews);
        ...
        private ModelView[] ReadViews(uint nViews, uint offset)
        {
            ...
            for(uint i = 0 ; i<nViews; i++)
            {
                ...
                file.ReadUInt32(); // uint nVertexProp = file.ReadUInt32();
                file.ReadUInt32(); // uint offVertexProp = file.ReadUInt32();

                file.ReadUInt32(); // uint nSubMesh = file.ReadUInt32();
                file.ReadUInt32(); // uint offSubMesh = file.ReadUInt32();

                file.ReadUInt32(); // uint nTexture = file.ReadUInt32();
                file.ReadUInt32(); // uint offTexture = file.ReadUInt32();
    ...
    class WDTFile : IDisposable
    {
        ...
        public WDTFile(StormDll.ArchiveSet archive, string name, 
                      WDT wdt, WMOManager wmomanager, ModelManager modelmanager)
        {
            string wdtfile = "World\\Maps\\" + name + "\\" + name + ".wdt";
            if (!archive.ExtractFile(wdtfile, "wdt.tmp")) return;
            ...
            stream = System.IO.File.OpenRead("wdt.tmp");
        ...
        public void LoadMapTile(int x, int y)
        {
            if (wdt.maps[x, y])
            {
                ...
                if (archive.ExtractFile(filename, "adt.tmp"))
                {
                    new MapTileFile("adt.tmp", t, wmomanager, modelmanager);
                    wdt.maptiles[x, y] = t;
                }
            }
        }
        ...
        private void HandleMODF(uint size)
        {
            ...
            for (uint i = 0; i < wdt.gnWMO; i++)
            {
                ...
                string path = wdt.gwmos[file.ReadInt32()]; //id];
    ...
    class DBCFile : IDisposable
    {
        ...
        {
            this.dbc = dbc;
            using (stream = System.IO.File.OpenRead(name))
            {
                using (file = new System.IO.BinaryReader(stream))
                {
                    bool done = false;
    ...
    class MapChunk
    {
        ...
        static readonly int[] holetab_h = new int[] { 0x1111, 0x2222, 0x4444, 0x8888 };
        static readonly int[] holetab_v = new int[] { 0x000F, 0x00F0, 0x0F00, 0xF000 };

        // 0 ..3, 0 ..3
        public bool isHole(int i, int j)
        {
            if(!hasholes) return false; 
            i /= 2; j /= 2;
    ...
    class MapTileFile : IDisposable // adt file
    {
        MapTile tile;
        System.IO.Stream stream;
        System.IO.BinaryReader file;
        ...
        static private void HandleMVER(uint size)
        {
        ...
        private void ReadMapChunk(MapChunk chunk)
        {
            ...
            bool debug = false; 
            ////Console.WriteLine("  " + zpos + " " + xpos + " " + ypos);
            bool done = false;
            do
            {
                try
                {                    
                    ...
                    {
                        size = sizeLiquid;
                        if (debug) //Console.WriteLine("MCLQ " + size);
                        if (sizeLiquid != 8) 
                        {
                            chunk.haswater = true;
                    ...
                    {
                        if (debug) //Console.WriteLine("MCSE " + size);
                        HandleChunkMCSE(chunk, size);
                    }
                    else if (type == ChunkReader.MCNK)
                    {
                        ////Console.WriteLine("MCNK " + size);
                        done = true; // found next
                        //HandleChunkMCSE(chunk, size);
                    }
                    else
                    {
                        ////System.Console.WriteLine("MapChunk Unknown " + type);
                        //done = true; 
                    }
                    file.BaseStream.Seek(curpos + size, System.IO.SeekOrigin.Begin);
                }
        ...
        static void HandleChunkMCNR(MapChunk chunk, uint size)
        {
            // Normals
        }

        void HandleChunkMCVT(MapChunk chunk, uint size)
        {
            // vertices
            int off = 0;
            for (int j = 0; j < 17; j++)
            {
                for (int i = 0; i < ((j % 2 != 0) ? 8 : 9); i++)
                {
                    ...
                    chunk.vertices[off++] = chunk.xbase + xpos; // x;
                    chunk.vertices[off++] = chunk.ybase + h; // y;
                    chunk.vertices[off++] = chunk.zbase + zpos; // z;                    
                }
            }
        }

        static void HandleChunkMCRF(MapChunk chunk, uint size)
        {

        }

        static void HandleChunkMCLY(MapChunk chunk, uint size)
        {
            // texture info
        }

        static void HandleChunkMCSH(MapChunk chunk, uint size)
        {
            // shadow map 64 x 64
        }


        static void HandleChunkMCAL(MapChunk chunk, uint size)
        {
            // alpha maps  64 x 64
        }

        void HandleChunkMCLQ(MapChunk chunk, uint size)
        {
            ...
            for (int i = 0; i < 9 ; i++)
            {
                for (int j = 0; j < 9; j++)
                {
                    file.ReadUInt32(); //UInt32 word1 = file.ReadUInt32();
        ...
        static void HandleChunkMCSE(MapChunk chunk, uint size)
        {
            // Sound emitters
        }
    ...
    class WmoRootFile : IDisposable
    {
        ...
        {
            file.ReadUInt32(); //uint nTextures = file.ReadUInt32();
            uint nGroups   = file.ReadUInt32();
            file.ReadUInt32(); //uint nP        = file.ReadUInt32();
            file.ReadUInt32(); //uint nLights   = file.ReadUInt32();
            wmo.nModels   = file.ReadUInt32();            
            wmo.nDoodads  = file.ReadUInt32();
            wmo.nDoodadSets   = file.ReadUInt32();

            file.ReadUInt32(); //uint col = file.ReadUInt32();
            file.ReadUInt32(); //uint nX    = file.ReadUInt32();
        ...
        private void HandleMODS(uint size)
        {
            ...
            {
                file.ReadBytes(20); //byte[] name = file.ReadBytes(20); // set name; 
...
0x24 	4 * uint8 	(B,G,R,A) color. Unknown. It is often (0,0,0,255). (something to do with lighting maybe?)
            ...
            for (int i = 0; i < sets /*wmo.nDoodads*/; i++)
            {
                ...
                Vec3D dir = new Vec3D(quatx, quaty, quatz);
        ...
        private void HandleMOGI(uint size)
        {
            for (int i = 0; i < wmo.groups.Length; i++)
            {
                ...
                file.ReadUInt32(); //uint nameOfs = file.ReadUInt32();
            }
        }
    ...
    class WmoGroupFile : IDisposable
    {
        System.IO.Stream stream;
        System.IO.BinaryReader file;
        WMOGroup g;
 ...
 */
        ...
        private void HandleMOGP(uint size )
        {
            ...
            file.ReadSingle(); //float bound2X = file.ReadSingle();
            file.ReadSingle(); //float bound2Y = file.ReadSingle();
            file.ReadSingle(); //float bound2Z = file.ReadSingle();
            ...
            file.ReadUInt32(); //uint unknown1 = file.ReadUInt32();
            g.id = file.ReadUInt32();
            file.ReadUInt32(); //uint unknown2 = file.ReadUInt32();
            file.ReadUInt32(); //uint unknown3 = file.ReadUInt32();
        }
